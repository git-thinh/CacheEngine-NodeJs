...
const ___log_post_api = (...agrs) => console.log(...agrs);
const ___log = (...agrs) => console.log(...agrs);
//    ___log('\nMAIN: Start ...'); 

const ___log_tcp = (...agrs) => {
    // console.log(...agrs);
}

const ___log_tcp_db2cache = (...agrs) => {
    //console.log(...agrs);
}

const ___getCurentDate = function (option) {
    var today = new Date();
    var date = '';
...
const savelog_pos = function (type, data) {
    ...
    const file_name = "log_pos_" + ___getCurentDate() + '.txt';

    var text = '\n' + new Date().toLocaleString() + ' --> Type=' + type + '\n' + '--> Para= ' + data + '\r\n';

    _FS.appendFile('log_pos/' + file_name, text, (err) => {
        if (err) throw err;
    });
};
...
const savelog_affilate = function (type, data) {
    ...
    const file_name_affilate = "log_affilate_" + ___getCurentDate() + '.txt';

    var text = '\n' + new Date().toLocaleString() + ' --> Type=' + type + '\n' + '--> Para= ' + data + '\r\n';

    _FS.appendFile('log_aff/' + file_name_affilate, text, (err) => {
        if (err) throw err;
    });
};
...
const db___execute_callback = function (socket_response, request_, store, data, callback_ok, callback_err) {
    ...
    if (errs.length > 0) {
        ...
        //////if (store.toLowerCase().indexOf('biz_themdangky') == -1) {
        //////    cmd.execute(store, (err_, result) => {
        //////        if (err_) {
        //////            ___log_post_api(store + ' -> EXE_ERROR = ', err_);
        //////            //
        ...
        cmd.on('row', row => {
            ...
            //chunks.push(rm);
            //___log_post_api(store + ' -> CMD_ROW = ', rm);

            ___MSG_DB2CACHE.push({ client: null, text: '!' + JSON.stringify(rm) });
        });

        cmd.on('error', err_ => {
            //___log_post_api(store + ' -> CMD_ERROR = ', err_);
        });
...
new _JOB('* * * * * *', function () {
    if (___MSG_DB2CACHE.length > 0) {
        ...
        if (m) {
            ...
            ___log_tcp_db2cache('JOB ___MSG_DB2CACHE = ', text);
            if (text && text.length > 36) {
                const s = text.substr(1);

                //console.log('___MSG_DB2CACHE_TEXT = ' + s);
                //console.log('\r\n\r\n');
                //return;
                ...
                if (s[0] == '{' && s[s.length - 1] == '}') {
                    ...
                    if (validJson) {
                        ...
                        switch (key) {
                            case 'CACHE_ALL_USER':
                                if (___cache['USER']) {
                                    ...
                                    //if (socket) socket.write(JSON.stringify({ ok: true, message: 'Data user ' + para + ' sync success' }));
                                }
                        ...
                        ___log_tcp_db2cache('JOB ___MSG_DB2CACHE RESULT = ' + key, sync_ok, text);
    ...
    return {
        ...
        USER: "SELECT  u.UserID as id, \
                ...
                WHEN ug.GroupID = 44  THEN N'Hỗ trợ khách hàng' else  (select top(1) s.[Name] from pos.[GroupShop] gs  inner JOIN pos.[ShopDetail] s ON gs.ShopCode = s.Code where g.GroupID = gs.GroupID) \
                        ...
                        (case u.UserID when 617 then 1  when 1810  then 1 else 0 end) as bit_admin_caller \
...
_HTTP_APP.post('/user_call/config/:token', function (req, res) {
    ...
    if (token == ___TOKEN_API && data && data.length > 0) {
...
_HTTP_APP.get('/job/POL_PAWN/EXCEL_STATUS_YEAR_REMAIN/:from_date/:to_date/:csv', (req, res) => {
    ...
    for (var i = 0; i < ___data.length; i++) {
        ...
        if (___data[i].group_id == 44) {
            //console.log('sss====', ___data[i].___caller_online.str_group_name);
            obj___.str_shop_name = ___data[i].___caller_online.str_group_name;// tên bộ phận xử lý
            obj___.shop_id = ___data[i].___caller_online.group_id;// mã hỗ trợ khách hàng
        }
        else {
        ...
        if (___data[i].int_trans_to_shop_date != -1 && ___data[i].int_trans_to_shop_date != null) {
            date_int_trans_to_shop_date = ___data[i].int_trans_to_shop_date.toString().substr(6, 2) + '-' + ___data[i].int_trans_to_shop_date.toString().substr(4, 2) + '-' + ___data[i].int_trans_to_shop_date.toString().substr(0, 4);
        }
        else {
...
_HTTP_APP.post('/api/login', (req, res) => {
    ...
    //   const user_ = _.find(___cache['USER'], function (o) { return o.str_user_name == data.str_user_name && o.str_pass == data.str_pass_word; });    // chay that thi mo cai nay khong cho login bang pass mac dinh
    if (user_) {
...
// ',619,707,927,1508,1208,1207,1206,1786,';

let J_CONFIG_ACCOUNT = {
    "id": ",619,707,927,1508,1208,1207,1206,1786,"
};
